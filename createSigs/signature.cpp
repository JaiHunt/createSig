...
short* find_sig(char* term, hash_term** vocab_)
{
    hash_term* entry;
    auto vocab = *vocab_;
...
void signature_add(char* term, hash_term** vocab)
{
    short* term_sig = find_sig(term, vocab);
...
void compute_signature(char* sequence, int length, hash_term** vocab)
{
    memset(doc_sig, 0, sizeof(doc_sig));
    for (int i = 0; i < length - WORDLEN + 1; i++)
        signature_add(sequence + i, vocab);
...
void partition(char* sequence, int length)
{
    #pragma omp parallel
    {
        static hash_term* vocab = NULL;
        #pragma omp for
        for (int i = 0; i < (length + ((-1/2) * PARTITION_SIZE)); i += PARTITION_SIZE / 2) {
            compute_signature(sequence+i, min(PARTITION_SIZE, length-i), &vocab);
        }
        doc++;
    }
}
